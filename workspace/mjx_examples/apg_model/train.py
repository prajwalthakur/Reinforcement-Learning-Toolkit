...
import networks as apg_networks
...
class TrainingState:
    ...
    optimizer_state: optax.OptState   # holds the internal state of an optimizer. momentum terms , adaptive learning rate accumalators
...
def train(
    ...
    if wrap_env:
        if wrap_env_fn is not None:
            ...
            wrap_for_training = envs.training.wrap  #TODO: change episode_length, action_repeat : return vmapped
    ...
    def env_step(
        ...
        env_state,key = carry
        key, key_samaple = jax.random.split(key)
        actions = policy(env_state.obs,key_samaple)[0]
        new_state = env.step(env_state,actions)
        return (new_state,key), (new_state.reward, env_state.obs)
    
    def loss(policy_params, normalizer_params, env_state, key):
        f = functools.partial(
            env_state, policy = make_policy((normalizer_params, policy_params))
        )
        (state_h, _ ), (rewards,obs) = jax.lax.scan(
            f, (env_state,key),(jnp.arange(horizon_length//action_repeat))
            )
        return -jnp.mean(rewards), (obs,state_h) # obslist and final state
    ...
    def clip_by_global_norm(updates):
        ...
        return jax.tree_util.tree_map(
            lambda t: jnp.where(trigger,t, (t/g_norm)*max_gradient_norm),  # value drawn from t if trigger is true otherwise from y
            updates,
        )
    ...
    policy_params =  apg_networks.policy_network.init(global_key)
    del global_key    
    ...
    # prevent recompilation on the seond epoch
